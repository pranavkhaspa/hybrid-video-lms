...
def concatenate_videos(video_paths: List[str], master_audio_path: str, output_path: str):
    ...
    try:
        ...
        is_mismatch = any(
            m["width"] != first_meta["width"] or
            m["height"] != first_meta["height"] or
            m["fps"] != first_meta["fps"] or
            m["codec_name"] != first_meta["codec_name"] or
            m["pix_fmt"] != first_meta["pix_fmt"]
            for m in metadatas
        )
        ...
        if not is_mismatch:
            ...
            concat_list_path = os.path.join(os.path.dirname(output_path), f"concat_list_{int(time.time())}.txt")
            ...
            filter_parts.append(f"{concat_v_inputs}concat=n={len(video_paths)}:v=1:a=0[vout]")
