...
import { AlertTriangle, Code, Layout, RefreshCw } from 'lucide-react';

export const Scene3Problems = ({ currentTime }) => {
  const elapsed = currentTime - 120; // 0 to 60s
  
  // Highlight different hotspots dynamically based on voiceover timing
  const activeIndex = 
    elapsed < 15 ? 0 :
    elapsed < 30 ? 1 :
    elapsed < 45 ? 2 : 3;
